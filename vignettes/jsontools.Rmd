...
|  | `RJSONIO` | `rjson` | `jsonlite` |
|------:|:-----|---------|:------:|
| JSON vector | silently takes first element | silently takes first element | collapses to single string, then parser error | 
| `NA` input | unclear error | unclear error | unclear error |
| `NULL` input | unclear error | unclear error | unclear error |


* they are not vectorized. `RJSONIO` and `rjson` silently use only the first element of the input; `jsonlite` collapses the input and then complains about trailing garbage.
* `NA` and `NULL` input produce unclear errors and one cannot control how to handle them.
* the camel case syntax doesn't fit to the rest of the tidyverse.
* converting a dataframe row by row to a vector of JSON is difficult.
