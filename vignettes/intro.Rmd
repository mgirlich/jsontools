...
To find out who commited we use `jsonr_extract_chr()`

```{r}
jsonr_extract_chr(gh_commits_vec, ".commit.committer.name")
```
...
commits_df <- commits_df %>% 
  mutate(
    sha = jsonr_extract_chr(json, ".sha"),
    committer_name = jsonr_extract_chr(gh_commits_vec, ".commit.committer.name"),
    message = jsonr_extract_chr(gh_commits_vec, ".commit.message")
  ) %>% 
  select(-json, everything())
commits_df
```
...
```{r}
emails <- jsonr_extract_chr(gh_commits_vec, ".commit.committer.email")
...
gh_commits_vec %>% 
  json_build_object(
    ...
    dates = "[.commit.committer.date, .commit.author.date]"
  )
```
